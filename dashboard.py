...
# ══════════════════════════════════════════════════════════════════════════════
# PAGE 5 — Monitoreo (Solarview / Solenium)
# ════════��═════════════════════════════════════════════════════════════════════
elif page == "Monitoreo":

    st.markdown("""
    <div class="page-heading">Monitoreo de Generación</div>
    <div class="page-subheading">Datos de generación solar en tiempo real desde la infraestructura Solenium.</div>
    """, unsafe_allow_html=True)

    bank_names_mon = [b["name"] for b in banks]
    mon_tabs = st.tabs(bank_names_mon)

    for bank_idx_m, tab in enumerate(mon_tabs):
        with tab:
            bank_name = bank_names_mon[bank_idx_m]
            projects = SOLARVIEW_PROJECTS.get(bank_name, [])

            if not projects:
                st.info(f"No hay proyectos de monitoreo configurados para **{bank_name}**.")
                continue
            ...
            scale = st.radio(
                ...
                ["Mensual", "Diario"],
                horizontal=True,
                key=f"mon_scale_{bank_idx_m}",
            )
            scale_val = "month" if scale == "Mensual" else "day"

            # ── Try to fetch real data, fallback to mock ──────────────
            token = st.session_state.solarview_token
            real_data = {}
            using_real = False

            if token:
                for proj in projects:
                    data = fetch_solarview_data(proj["id"], scale_val, token)
                    if data:
                        real_data[proj["name"]] = data
                        using_real = True

            if using_real and real_data:
                st.success("Datos en vivo desde Solenium")
                project_data = real_data
            else:
                if token:
                    st.warning("No se pudo conectar a la API. Mostrando datos simulados.")
                else:
                    st.info("Configure el Token Solarview en Configuración API para datos reales. Mostrando datos simulados.")
                project_data = generate_solarview_mock(projects, scale_val)

            # ── Summary KPIs ──────────────────────────────────────────
            total_gen = 0
            for pname, pdata in project_data.items():
                if isinstance(pdata, dict) and "energy_kwh" in pdata:
                    total_gen += sum(pdata["energy_kwh"])

            num_plants = len(projects)
            total_cap = sum(p["cap_kw"] for p in projects)

            mk1, mk2, mk3 = st.columns(3)
            with mk1:
                st.markdown(f'<div class="metric-card"><div class="label">Generación Total</div><div class="value">{total_gen/1000:,.0f}</div><div class="sub">MWh ({scale.lower()})</div></div>', unsafe_allow_html=True)
            with mk2:
                st.markdown(f'<div class="metric-card"><div class="label">Plantas</div><div class="value">{num_plants}</div><div class="sub">Proyectos activos</div></div>', unsafe_allow_html=True)
            with mk3:
                st.markdown(f'<div class="metric-card"><div class="label">Capacidad</div><div class="value">{total_cap/1000:.1f}</div><div class="sub">MW instalados</div></div>', unsafe_allow_html=True)
            ...
            if HAS_PLOTLY:
                ...
                MON_AXIS = "#787600"
                MON_GRID = "rgba(168,166,12,0.15)"

                for pi, (pname, pdata) in enumerate(project_data.items()):
                    if isinstance(pdata, dict) and "energy_kwh" in pdata:
                        labels = pdata.get("labels", list(range(len(pdata["energy_kwh"]))))
                        energy_mwh = [v / 1000 for v in pdata["energy_kwh"]]
                        c = colors_palette[pi % len(colors_palette)]
                        fig_mon.add_trace(go.Scatter(
                            x=labels, y=energy_mwh,
                            name=pname, mode="lines",
                            line=dict(width=1, color=c),
                            fill="tonexty" if pi > 0 else "tozeroy",
                            fillcolor=c,
                ...
                period_label = "Mes" if scale_val == "month" else "Día"
                fig_mon.update_layout(
                    title=dict(
                        text=f"Generación Solar — {bank_name} ({scale})",
                        font=dict(family="Poppins, sans-serif", size=15, color=MON_AXIS),
                        x=0, xanchor="left", y=0.98, yanchor="top",
                    ),
                    plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
                    font=dict(family="Lato, sans-serif", size=11, color=MON_AXIS),
                    margin=dict(l=55, r=155, t=40, b=30), height=460,
                    legend=dict(
                        orientation="v",
                        yanchor="top", y=1,
                        xanchor="left", x=1.03,
                        font=dict(size=10, color=MON_AXIS),
                        bgcolor="rgba(0,0,0,0)",
                        borderwidth=0,
                        tracegroupgap=2,
                    ),
                    yaxis=dict(gridcolor=MON_GRID, zeroline=False,
                               title=dict(text="MWh", font=dict(color=MON_AXIS, size=11)),
                               tickfont=dict(color=MON_AXIS, size=10)),
                    xaxis=dict(showgrid=False,
                               title=dict(text=period_label, font=dict(color=MON_AXIS, size=11)),
                               tickfont=dict(color=MON_AXIS, size=10)),
                    hovermode="x unified",
                )
                st.plotly_chart(fig_mon, width="stretch", config={"displayModeBar": False})
            else:
                st.info("Instala plotly para gráficos: `pip install plotly`")

            # ── Per-project breakdown table ───────────────────────────
            st.markdown('<div class="section-title">Detalle por Planta</div>', unsafe_allow_html=True)

            for proj in projects:
                pdata = project_data.get(proj["name"], {})
                if isinstance(pdata, dict) and "energy_kwh" in pdata:
                    total_kwh = sum(pdata["energy_kwh"])
                    avg_kwh = total_kwh / max(len(pdata["energy_kwh"]), 1)
                    cap_factor = (avg_kwh / (proj["cap_kw"] * 24 * (30 if scale_val == "month" else 1))) * 100 if proj["cap_kw"] > 0 else 0
                else:
                    total_kwh = 0
                    avg_kwh = 0
                    cap_factor = 0
                ...
                st.markdown(f"""
                <div style="background:#fff; border:1px solid var(--u-purple-15); border-radius:12px; padding:0.8rem 1.2rem; margin-bottom:0.4rem; display:flex; justify-content:space-between; align-items:center;">
                    <div>
                        <div style="font-family:'Poppins',sans-serif; font-size:0.85rem; font-weight:700; color:var(--u-deep);">{proj["name"]}</div>
                        <div style="font-size:0.68rem; color:#6b7280; font-weight:600;">{proj["cap_kw"]} kWp instalados</div>
                    </div>
                    <div style="display:flex; gap:1.5rem; align-items:center;">
                        <div style="text-align:center;">
                            <div style="font-family:'Poppins',sans-serif; font-size:1rem; font-weight:800; color:var(--u-purple);">{total_kwh/1000:,.1f}</div>
                            <div style="font-size:0.58rem; color:#6b7280; text-transform:uppercase; font-weight:700;">MWh Total</div>
                        </div>
                        <div style="text-align:center;">
                            <div style="font-family:'Poppins',sans-serif; font-size:1rem; font-weight:800; color:var(--u-deep);">{cap_factor:.1f}%</div>
                            <div style="font-size:0.58rem; color:#6b7280; text-transform:uppercase; font-weight:700;">Factor Planta</div>
                        </div>
                    </div>
                </div>
